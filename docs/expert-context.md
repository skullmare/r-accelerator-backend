...
**Обратная связь агенту.** Вердикт оценщика — не только гейт и состояние
кнопки: он подмешивается в системный промпт агента на каждом ходу
(`buildStageStatusPrompt` в `expert-session.service.js`). Без этого агент и
оценщик — два независимых LLM-вызова, не знающих друг о друге: агент вольно
интерпретировал свои `completionCriteria` и прощался («удачи на следующем
этапе!»), пока оценщик держал этап неготовым — пользователь видел на одном
экране два противоречащих сообщения. Теперь при `ready:false` агент получает
список несобранных полей + `reason` оценщика и явный запрет объявлять этап
завершённым; при `ready:true` — разрешение предложить пользователю кнопку
«Завершить этап» (но не «завершить» самому). Статус берётся с прошлого хода
(оценка выполняется после ответа агента) — лаг в одно сообщение осознанный:
свежие реплики пользователя агент и так видит в истории.
