 ...
 *           description: Статус проекта. Меняется вручную через PATCH, кроме одного автоматического перехода — сервер сам ставит "completed", когда подтверждён артефакт последнего агента маршрута (nextAgentId отсутствует). Не влияет на доступ к экспертному маршруту — это отражение состояния, а не гейт.
 *         progress:
 *           type: number
