...
const AgentSchema = new mongoose.Schema({
    ...
    // Ссылка на «думающую» аватарку агента — отдельное изображение для
    // состояния, когда модель формирует ответ (индикатор размышления в UI).
    // Необязательное, в промпт модели не подмешивается.
    thinkingAvatarUrl: {
        type: String,
        trim: true,
        default: null,
    ...
    // Приветственное сообщение агента, которое UI показывает пользователю в
    // начале диалога (до первого его сообщения). Необязательное, в промпт
    // модели не подмешивается — чисто отображаемое.
    greeting: {
        type: String,
        trim: true,
        default: null,
    ...
    // УСТАРЕЛО. Промпт отдельной модели-«оценщика», которая после каждого хода
    // решала, собрана ли информация для артефакта. Оценщик удалён: агент сам
    // складывает данные в карточку этапа (ExpertSession.collectedFields) через
    // инструмент save_collected_fields, а готовность считается арифметикой по
    // заполненным полям (field-collection.service.js). Поле оставлено, чтобы не
    // ломать админский API и уже сохранённые конфигурации агентов, но на
    // поведение больше не влияет — управляйте сбором через systemPrompt,
    // completionCriteria и artifactDefinition.requiredFields.
    ...
    // Разрешать ли завершение этапа с частично заполненным артефактом
    // (когда собраны не все данные). false — требуется полнота по критериям.
    ...
    modelConfig: {
        ...
        // Дешёвая вспомогательная модель. После удаления оценщика готовности
        // используется только для разового бэкфилла карточки этапа в сессиях,
        // начатых до перехода на save_collected_fields
        // (backfillCollectedFields). Если не задана — используется model выше.
